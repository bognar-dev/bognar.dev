...
function Dot({ mousePos }: { mousePos: { x: number, y: number } }) {
    ...
    return (
        <div ref={dotRef} className="relative">
            <motion.div className="bg-accent-600 rounded-full absolute -translate-x-1/2 -translate-y-1/2"
...
export default function BGBalls({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="container min-h-100% mx-auto p-4 relative ">
            <div className="flex flex-wrap  gap-10 md:gap-24 mx-auto p-12">
                {
                    Array.from({ length: width * 0.05 }, (_, i) => (
                        <Dot key={i} mousePos={mousePos}></Dot>
                    ))
                }
            </div>
            <div className="w-1/2 h-1/2 absolute top-1/4 left-1/4 flex flex-col justify-center items-center">{children}</div>
        </div>
    )
}
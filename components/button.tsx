...
export default function Button({ className, children, href }: { className?: string, children: React.ReactNode, href?: string }) {
    ...
    return (
        <Link href={href} className={twMerge('text-text-50 text-sm font-medium  py-2 px-4 rounded-md bg-primary-500 mt-10 shadow-sm shadow-primary-200 hover:shadow-primary-500 hover:translate-x-1 transition-all justify-self-center', className)}>
            <button >
                {children}
            </button>
        </Link>

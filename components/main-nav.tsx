  ...
  return (
    <div className=" sticky top-0 z-40 w-full bg-white/10 backdrop-blur-sm rounded-b-sm">
      <div className="sticky top-0 z-50 flex flex-col gap-2 min-w-screen items-center justify-between divide-y-2 hover:divide-primary-300 divide-primary-100 pb-3 rounded-xl">
        ...
        <Link href="/" className="font-head text-4xl flex items-center justify-items-center justify-center font-medium bg-opacity-10 hover:translate-x-1 hover:bg-secondary-200  ease-out duration-100 rounded-md px-4 mt-2">Bognar.dev</Link>
        {items?.length ? (
              ...
              (item, index) =>
                item.href && (
                  <Link
                    ...
                    href={item.href}
                    className=
                    "text-xs sm:text-md lg:text-xl group flex items-center font-medium hover:translate-x-1 hover:bg-secondary-200 ease-out duration-100 rounded-md md:px-4 mt-1 py-1"
                  >
                    {item.title}
